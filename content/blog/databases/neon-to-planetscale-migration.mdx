...
- [ ] All of your Postgres extensions you depend on are supported by PlanetScale (check their [extensions documentation](https://planetscale.com/docs/postgres/extensions)). Make sure to enable these before starting the migration to be extra safe!
...
Log in to the [PlanetScale dashboard](https://app.planetscale.com/) and click "Create a new database".
...
- **Storage options**:
    - [**PlanetScale Metal**](https://planetscale.com/metal): For applications needing high-performance and low-latency I/O. **You'll need to choose your storage size during creation (now)**. (Don't worry, you can resize it later!)
...
<Callout title="If you choose PlanetScale Metal…." type="warning">
	...
	After importing and cleaning up table bloat, you may be able to downsize to the 468 GB option (as per the example). **Don't worry! Resizing is a no-downtime operation. More on that in the PlanetScale [Cluster configuration](https://planetscale.com/docs/postgres/cluster-configuration#adjusting-cluster-size) docs.**
</Callout>
...
<Callout title="Skip condition" type="error">
	If you get an empty table, you can safely skip this section (to [Moving reads & writes to PlanetScale](#moving-reads--writes-to-planetscale)) since you do not have anything that uses sequences.
</Callout>
...
You can downsize your database after the migration as described [here](https://planetscale.com/docs/postgres/cluster-configuration#adjusting-cluster-size).
