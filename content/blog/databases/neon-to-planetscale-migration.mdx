...
This guide walks through migrating a 2.7M record database with foreign keys, auto-incrementing IDs, and unique constraints—the parts that usually break during migrations. The same SQL and methods scale regardless of whether you have 2 tables or 200.
	
For production examples: [OpenSecret](https://opensecret.cloud/) used [PlanetScale's migration scripts](https://github.com/planetscale/migration-scripts) based on [their migration guide](https://planetscale.com/blog/postgres/imports/neon#3-prepare-the-neon-database) for their migration. You can read their [blog post](https://blog.opensecret.cloud/why-we-migrated-from-neon-to-planetscale/) about it. This guide follows the same documentation, with the scripts available as automation options.
	
You can find many more [case studies done by PlanetScale](https://planetscale.com/case-studies), including how those companies migrated.
...
*But for real: In my opinion, a migration this way takes very little effort; it's literally the most straightforward thing ever. You can easily do it with no previous migration experience. Getting the 2 DBs setup and getting the database credentials took more effort than the migration itself!*
...
- [ ] You have the necessary permissions for the 2 databases to perform the steps:
  - [ ] **PlanetScale**: dashboard access with `Database Administrator` or `Organization Administrator` role to create the new database, change its settings, and connect to it
  - [ ] **Neon**: dashboard access with `Collaborator` role or above to change its settings, and connect to it
...
<Callout title="If you choose PlanetScale Metal…." type="warning">
	...
	Tip: not all sizes are available on both architectures; the storage size value in the cluster type cards are dropdowns!
...
This will launch the role creation wizard. We'll now create our role, which we'll use for our migration. It is considered best practice to not use one role with high-level permissions for everything.
...
The PlanetScale docs say that you may see errors in the format of `psql:schema.sql:LINE: ERROR: DESCRIPTION`. I haven't encountered this myself in the couple of simulated migrations I've done, but if you find anything concerning, contact PlanetScale before continuing.
...
This should output something like `CREATE PUBLICATION{:sql}` if run correctly.
...
Congrats! Your PlanetScale database is now catching up with your Neon database. As I wrote above, this is near instant for small databases. But even if it's not, we can work on migrating sequences while it's copying / replicating.
...
<Callout title="Skip condition" type="error">
	If you get an empty table, you can safely skip this section (to [Moving reads & writes to PlanetScale](#moving-reads--writes-to-planetscale)) since you do not have anything which uses sequences.
</Callout>
...
<Callout title="If you do not know how to measure, or don't want to guess this yourself" type="info">
	It might be beneficial to use the [`ff-seq.sh` script](https://github.com/planetscale/migration-scripts/blob/main/postgres-direct/ff-seq.sh) provided by PlanetScale, which, when provided with your Neon database credentials (`--primary`), PlanetScale credentials (`--secondary`), and a `--skip` property in seconds, will sample your database and fast-forward the sequences on your PlanetScale database enough to only catch up by the amount of seconds provided.
	...
	**If you know your constraints, the solution is straightforward. Assume 100 records inserted per second and a 5-hour migration window. You can calculate the total records: $$100\,\frac{\text{records}}{\text{sec}} \times 18,000\,\text{sec} \, (5\,\text{hours}) = 1{,}800{,}000$$. Then fast-forward your sequences by that amount.**
...
FROM pg_sequences;
```
<Callout title="PlanetScale docs inaccuracy" type="warning">
	...
	The corrected version above properly quotes only the sequence name and uses `::regclass` to resolve it correctly.
	...
	LINE 1: SELECT setval("public.User_id_seq", 55000);
	```
</Callout>
...
Now for verification, [check the query we ran on Neon](#handling-sequences-if-you-even-use-those), but now on PlanetScale, it should report the newly set values:
...
(It's totally possible for the row counts to match, but for the LSNs to not.)
...
```sql
DROP SUBSCRIPTION replicate_from_neon;
```

If that ran successfully, run this on Neon to clear the publication:

```sql
DROP PUBLICATION replicate_to_planetscale;
```
...
Debloating is out of scope for this guide, but this is very well documented by PlanetScale. You're probably going to want to use something like `pg_squeeze`. For which documentation is available [here](https://planetscale.com/docs/postgres/extensions/pg_squeeze).

### Downsizing

You can downsize your database after the migration as described [here](https://planetscale.com/docs/plans/planetscale-skus#upsizing-and-downsizing-clusters).
